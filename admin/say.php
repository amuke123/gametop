...
if(ADMIN_URL!==null){$pagenum = Control::get('admin_page_num');}
...
if(isset($_POST['add'])){
	...
	$times = time();
	$sqlre = "INSERT INTO `". DB_PRE ."say` (`id`,`a_id`,`top_id`,`t_id`,`date`,`posterid`,`content`,`mail`,`url`,`ip`,`show`,`ischeck`,`mark`) VALUES (NULL,'".$aid."','".$topid."','".$tid."','".$times."','".UID."','".$desc."','".$user_cache[ UID ]['mail']."','". IDEA_URL ."','".getIp()."','1','1','0');";
	if($db->query($sqlre)){
		$sql7="SELECT count(*) as sumnum FROM `". DB_PRE ."say` WHERE `del`='1' and `a_id` = ".$aid.";";
		$arr=$db->getlist($sql7);
		$sql8="UPDATE `". DB_PRE ."article` SET `saynum`='".$arr[0]['sumnum']."' WHERE `id` = ".$aid.";";
		$db->query($sql8);
		updateCacheAll(array('sta','newArts','goods','eyes','collect'));
		echo "<script>location.href='". ADMIN_URL ."say.php';</script>";
	}
	exit();
}
...
if($action=='edit'){
	...
	$sqlline="SELECT * FROM `". DB_PRE ."say` WHERE `id`=".$eid;
	$lineedit=$db->getlist($sqlline);
}

$sqltem="";
if($god=='0'){$sqltem.=" AND `mark`='2' ";}
if($good=='0'){$sqltem.=" AND `mark`='1' ";}
if($examine=='0'){$sqltem.=" AND `ischeck`='0' ";}
