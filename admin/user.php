...
$pagenum = Control::get('admin_page_num');//每页显示用户数
...
if($action=='check'){
	...
	$sqlst="UPDATE `". DB_PRE ."user` SET `ischeck`='1' WHERE `id`=".$idck;
	if($db->query($sqlst)){
		updateCacheAll('sta');
		echo "<script>location.href='".ADMIN_URL ."user.php?examine=0'</script>";exit;
	}
}
if($action=='state'){
	...
	if($db->query($sqlst)){
		updateCacheAll('sta');
		echo "<script>location.href='".ADMIN_URL ."user.php?state=0'</script>";exit;
	}
}
if($action=='do'){
...
if(isset($_POST['add'])||isset($_POST['edit'])){
	...
	if($ajcode==$_SESSION['ajcode']){
		...
		if(isset($_POST['edit'])){
			...
			$cxjg=$db->getlist($sqlcx);
			$password=setUTF8(Checking::jm($oldpw));
			if(!empty($cxjg[0]['password'])&&$oldpw!=''){
				if(Checking::checkPw($password,$cxjg[0]['password'])){
					if(!isset($_POST['password1'])||$_POST['password1']==''){
						echo "<script>alert('密码不能为空！');window.history.back();</script>";exit;
					}else{
						$datauser['password']=Checking::hashjm(Checking::jm($_POST['password1']));
