	...
	<?php if($action=='edit'){?>
		...
		<div class="m_add2">
			<form action="" method="post" name="sayadd" onSubmit="return yzSayAdd();" >
				<input type="hidden" name="id" value="<?php echo $lineedit[0]['id']?>" />
				<input type="hidden" name="tid" value="<?php echo $lineedit[0]['t_id']?>" />
				<input type="hidden" name="aid" value="<?php echo $lineedit[0]['a_id']?>" />
				<p><b>评论人：</b><?php echo $lineedit[0]['name'];?></p>
				<p><b>时间：</b><?php echo date("Y-m-d H:i:s",$lineedit[0]['date']);?></p>
				<p><b>内容：</b><?php echo $lineedit[0]['top_id']!='0'?"@".$lineedit[0]['name'].'：':'';echo mb_substr(strip_tags($lineedit[0]['content']),0,40);?></p>
				...
				<div class="m_list" id="m_list">
					...
					<p>
						...
						<span class="tleft linesort">
							...
							<?php echo $value['ischeck']=="0"?"<img class='iconpic' title='未审核' src='". ADMIN_URL ."/view/static/images/weishen.png' />":"";?> 
							<?php echo $value['mark']=="1"?"<img class='iconpic' title='加精' src='". ADMIN_URL ."/view/static/images/jing.png' />":"";?> 
							<?php echo $value['mark']=="2"?"<img class='iconpic' title='神评' src='". ADMIN_URL ."/view/static/images/shen.png' />":"";?> 
							<br />
						...
						<span><a href="javascript:showOrHide('<?php echo $value['id']."','".IDEA_URL."','";echo $value['show']==0?'1':'0';?>','say','<?php echo $value['a_id'];?>');"><?php echo $value['show']==0?'<img title="隐藏" src="'. ADMIN_URL .'/view/static/images/plugin_inactive.gif" />':'<img title="显示" src="'. ADMIN_URL .'/view/static/images/plugin_active.gif" />';?></a></span>
					</p>
					<?php }?>
				</div>
				<div class="m_set">
