			...
			<form action="" method="post" name="bannadd" onSubmit="return yzBannAdd2();" enctype="multipart/form-data" >
				<p>
					<select name="group" class="formsele">
						<?php for($i=0;$i<4;$i++){?>
							<option value="<?php echo $i;?>" <?php echo $banners[$bannid]['group']==$i?'selected':'';?> ><?php echo $groups[$i];?></option>
						<?php }?>
					</select>
				</p>
				<input type="hidden" name="id" value="<?php echo $banners[$bannid]['id']?>" />
				...
				<div class="group">
					<p><b>轮换图</b><img <?php echo $banners[$bannid]['pic']==""?'src="'.ADMIN_URL .'view/static/images/img.gif" title="无图"':'src="'.$banners[$bannid]['pic'].'"';?> /><input type="hidden" name="pic2" value="<?php echo $banners[$bannid]['pic']?>" /></p>
