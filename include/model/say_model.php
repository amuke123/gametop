...
class say_Model{
	static function getTipsSay($num,$sqlpro='',$order=''){
		$db=Conn::getConnect();
		$sql="SELECT * FROM `". DB_PRE ."say` WHERE `del`='1' AND `ischeck`='1' AND `show`='1' ";
		$sql.=$sqlpro;
	...
	static function getSays($artid,$sqltem,$startnum,$pagenum,$del='1',$order='DESC',$top='',$tid=''){
		$db=Conn::getConnect();
		$sql="SELECT * FROM `". DB_PRE ."say` WHERE 1=1 ";
		if($top!=''){$sql.=" AND `top_id`='".$top."' ";}
		if($del=='1'){$sql.=" AND `del`='".$del."' ";}else{$sql.=" AND `show`='1' AND `ischeck`='1' ";}
	...
	static function isCommentExist($aid,$name,$content){
		$db=Conn::getConnect();
		$data = $db->getOnce("SELECT COUNT(*) AS `total` FROM `".DB_PRE ."say` WHERE `a_id`='".$aid."' AND (`posterid`='".UID ."' OR `name`='".$name."') AND `content`='".$content."'");
		return intval($data['total'])>0?true:false;
	}
	static function getSayTid($pid){
		$db=Conn::getConnect();
		$data=$db->getOnce("SELECT `t_id` FROM `". DB_PRE ."say` WHERE `id`='".$pid."'");
		return $data['t_id'];
	}
	static function updateCommentNum($aid) {
        ...
        if(is_array($aid)){
            ...
            $sql="SELECT count(*) as `total` FROM `".DB_PRE ."say` WHERE `a_id`='".$aid."' AND `show`='1' AND `ischeck`='1'";
            $res=$db->getOnce($sql);
            $comNum=$res['total'];
	...
	static function addComment($name,$content,$mail,$url,$aid,$pid,$tid){
        ...
        $sql="INSERT INTO `".DB_PRE ."say` (`id`,`a_id`,`top_id`,`t_id`,`date`,`posterid`,`name`,`content`,`mail`,`url`,`ip`,`show`,`ischeck`,`mark`,`good`,`bad`,`del`) VALUES (NULL,'".$aid."','".$pid."','".$tid."','".$utctimestamp."','".UID ."','".$name."','".$content."','".$mail."','".$url."','".$ipaddr."','1','".$hide."','0','0','0','1')";
        $ret = $db->query($sql);
	...
	static function isCommentTooFast(){
        ...
        $sql = 'select count(*) as `total` from `'.DB_PRE ."say` where `date`>".$utctimestamp." AND ip='".$ipaddr."'";
        $row = $getOnce($sql);
	...
	static function getSayNum($artid='0',$sqltem='',$del='1',$show='',$topid=''){
		$db=Conn::getConnect();
		$sql2="SELECT count(*) as total FROM `" . DB_PRE . "say` WHERE 1=1 ";
		if($show!=''){$sql2.=" AND `show`='1' AND `ischeck`='1' ";}
	...
	static function getTopidUname($topid){
		$db=Conn::getConnect();
		$sql="SELECT `posterid` FROM `". DB_PRE ."say` WHERE `id` = ".$topid;
