...
class art_Control{
	function disIndex($params=array()){
		...
		$wishtop=wishlist_Model::getTopWhih();
		$artids=explode(',',$wishtop['artids']);
		$artone=art_Model::getOnceArt($artids[0]);
		$arttwo=art_Model::getOnceArt($artids[1]);
		$wishlists=wishlist_Model::getWhihlist(3,$wishtop['id']);
		$sjslist=user_Model::getUserList();
		$cache=Conn::getCache();
		$sorts=$cache->readCache('sort');
		$artSortList=$cache->readCache('artSortList');
		$says=say_Model::getTipsSay(Control::get('top_say_num')," AND `mark`='2' ","");
		$banners=$cache->readCache('banner');
		$newArts=$cache->readCache('newArts');
