...
class Upfile{
	...
	function show(){
		//echo $this->save_path."<br>";
		//echo $this->save_url."<br>";
		//print_r($this->ext_arr);
		//echo "<br>".$this->max_size."<br>";
	...
	function upload($file,$aid='',$dir_name='',$tem = 0,$fhtem=0){//file 文件，aid 所属文章，dir_name 非文章分类,$tem 是否生成缩略图,$fhtem 返回缩略图开关
		...
		$file_size = $file['size'];
		//检查文件名
		if(!$file_name){return $file_name."请选择文件。";}
		//检查目录
		if(@is_dir($this->save_path) === false){return $file_name."上传目录不存在。";}
		//检查目录写权限
		if(@is_writable($this->save_path) === false){return $file_name."上传目录没有写权限。";}
		//检查是否已上传
		if(@is_uploaded_file($tmp_name) === false){return $file_name."上传失败。";}
		//检查文件大小
		if($file_size > $this->max_size){return $file_name."上传文件大小超过限制。";}
		//获得文件扩展名
		...
		$new_path=$this->save_path;
		$new_url=$this->save_url;
		//新文件名
