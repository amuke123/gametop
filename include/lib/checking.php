...
class Checking{
	...
	public static function getLogin($username,$password,$code='',$save=0){//登陆验证
		...
		if(!empty($row)){
			...
			if(self::checkPw($password,$pass)){
				...
				echo "<script>alert('您的登陆密码错误或账户不存在！');window.history.go(-1);</script>";
			}
		}else{
			echo "<script>alert('您的登陆密码错误或账户不存在！');window.history.go(-1);</script>";
		}
	...
	public static function roleOk($role = ''){//权限认证
		$infos = user_Model::getInfo();
		if($role == ""||$role != ROLE_ADMIN){
			echo "<script>alert('权限不足！');location.href='". IDEA_URL ."';</script>";exit;
		}
	}
	
	public static function goIndex(){//跳转网站首页
		echo "<script>location.href='". IDEA_URL ."';</script>";
	}
	
	public static function goAdmin(){//跳转后台主页
		echo "<script>location.href='". IDEA_URL . ADMIN_TYPE ."';</script>";
	}
	
	public static function goLogin(){//跳转登陆页
		echo "<script>location.href='". Url::getActionUrl('login') ."';</script>";
	}
