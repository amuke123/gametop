...
class Email{
	static function sendMail($emailid,$data=''){//发送邮件
		...
		$sendmailhost = Control::get('mailhost');
		$sendmail = Control::get('mail');
		$sendmailpswd = Control::get('mailpswd');
		$sendmailport = Control::get('mailport');
		$sitename = 'IDEASHU';
		$code = Cellcode::getCode($emailid);
		$title = '验证码';
		$toemail = $emailid;
		$str = $data!=''?$data:'<p>您的验证码是：</p><p><b>'.$code.'</b></p><p>如非本人操作，请忽略此邮件，由此给您带来的不便请谅解！</p><p>IDEASHU</p>';
		...
		$mail->Host = $sendmailhost;// 链接qq域名邮箱的服务器地址
		$mail->SMTPSecure = 'ssl';// 设置使用ssl加密方式登录鉴权
		$mail->Port = $sendmailport;// 设置ssl连接smtp服务器的远程服务器端口号
