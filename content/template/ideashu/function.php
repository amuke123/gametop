...
function site_nav($sortid=0){
	...
	$tops_ca=Control::getTops();
	foreach($nav_cache as $value){
		if($value['top_id']!=0){continue;}
		if($value['show']==0){continue;}
		$blank=$value['blank']=='1'?'target="_blank"':'';
		if(!empty($value['children']) || !empty($value['childnav']) || !empty($value['list'])){
		?>
		<li>
		...
		<li <?php echo $value['id']==$sortid?'class="active"':'';?>>
			<?php if($value['id']=='5'){?>
				<a href="<?php echo IDEA_URL .$value['url']; ?>" <?php echo $blank;?>><?php echo $value['name']; ?>▾</a>
				<ul>
                    <?php foreach ($sort_cache as $rows){
                        echo '<li><a href="'.Url::sort($rows['id']).'">'.$rows['sortname'].'</a></li>';
                    }?>
                </ul>
			<?php }else if($value['id']=='6'){?>
				<a href="<?php echo IDEA_URL .$value['url']; ?>" <?php echo $blank;?>><?php echo $value['name']; ?>▾</a>
				<ul>
                    <?php foreach ($tops_ca as $rowt){
                        echo '<li><a href="'.IDEA_URL .$rowt['url'].'">'.$rowt['name'].'</a></li>';
                    }?>
                </ul>
			<?php }else{?>
			<a href="<?php echo IDEA_URL .$value['url']; ?>" <?php echo $blank;?>><?php echo $value['name'];?></a>
